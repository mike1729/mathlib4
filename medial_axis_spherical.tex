...
medial centre nonetheless reconstructs them depends on the global medial
axis, and we have not settled it. We therefore state the variable-positive
case as follows.
\begin{itemize}
\item \emph{Unconditional:} constant curvature $+1$
...
\item \emph{Open:} whether $\Rx=M\setminus\X$ holds for \emph{sparse}
$\X$ in \emph{variable} positive curvature. The only obstruction to the
margin argument is cut-shielding (Proposition~\ref{prop:obstruction}); a
sufficient condition for total reconstruction is its absence ---
$\mathrm{cut}(x_{0};\gamma)\ge\sup\{t:\ x_{0}\ \text{uniquely
nearest}\}$ along every ray, which on $\Sn$ holds because the cut distance
$\pi$ realises the diameter (Remark~\ref{rem:why}).
\end{itemize}
\end{remark}
...
\[
  \Rx^{g}=M\setminus\X .
\]
No spread, curvature, or genericity hypothesis is needed.
\end{theorem}
