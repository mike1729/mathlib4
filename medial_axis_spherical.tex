...
\begin{remark}[The capture theorem halves the constant and drops
compactness]\label{rem:densebetter}
The final note of the series strengthens Theorem~\ref{thm:dense} on
both hypotheses: on any \emph{complete} $M$ with
$\inj(M):=\inf_{q}\inj(q)>0$, already
$\sup_{M\setminus\X}d_{\X}<\inj(M)$ --- for $\delta$-dense $\X$,
$\delta<\inj(M)$ --- gives $\Rx=M\setminus\X$ \cite[Cor.~3.3]{C}.
Compactness entered only to preclude escape, which the spread bound
does by itself; and the constant halves because the capture argument
of \cite{C} needs the reach bound only at the first multi-geodesic
point it meets, not along a rival-forcing comparison of length
$\sim2\delta$.
\end{remark}
...
\[
  \Rx^{g}=M\setminus\X .
\]
No spread or curvature hypothesis is needed. The containment of $p$ in
the moving ball past intermediate cut points --- the one formerly
delicate step --- is proved unconditionally in the final note of the
series \cite[Thm.~3.1, Cor.~3.2]{C}; Remark~\ref{rem:containment}
records the scope of the elementary margin argument and the repair.
\end{theorem}
...
the arc ceases to minimise and the constant-margin identity breaks. This
gap is now closed: the final note of the series proves the containment
unconditionally on every complete manifold, with no genericity or
analyticity hypothesis, by following the ascent only while the minimising
geodesic to $\X$ is unique --- there $d_{\X}$ grows at unit rate and the
margin cannot shrink, through focal and conjugate points without
comment --- and \emph{stopping at the first approach} to $\Mx^{g}$, where
the accumulated margin is intact, instead of flowing through it
\cite[Thm.~3.1, Cor.~3.2]{C}. Theorem~\ref{thm:twogeo} therefore holds as
stated for every compact $M$, and indeed for every closed
$\X\neq\varnothing$: even $|\X|=1$, invisible to the two-point axis, is
reconstructed in full by the two-geodesic one.
\end{remark}
...
\textbf{Two-geodesic axis} $\Rx^{g}=M\setminus\X$ (no spread)
  & any compact $M$ & \textbf{Thm.~\ref{thm:twogeo}}, \cite{C}\\
